(function() {
	setTimeout(function() {
		...
		if (typeof analytics !== 'undefined' && analytics.user) {
			tagline.innerHTML = analytics.user().anonymousId() || 'A JavaScript AI that beats humans at battleship.';
		}
	}, 500);
...
function Game(size) {
	...
	this.createGrid();
	this.init();
	// Track with Session Replay properties
	var replayProps = window.getSessionReplayProps ? window.getSessionReplayProps() : {};
	analytics.track('Game Initialized', replayProps);
}
...
Game.prototype.checkIfWon = function() {
	if (this.computerFleet.allShipsSunk()) {
		var replayProps = window.getSessionReplayProps ? window.getSessionReplayProps() : {};
		analytics.track('Game Ended', Object.assign({
			Win: true,
...
Game.prototype.shootListener = function(e) {
	...
	var replayProps = window.getSessionReplayProps ? window.getSessionReplayProps() : {};
	analytics.track('Shot Fired', Object.assign({
		X: x,
		Y: y,
...
Game.prototype.rosterListener = function(e) {
	...
	var replayProps = window.getSessionReplayProps ? window.getSessionReplayProps() : {};
	analytics.track('Ship Selected', Object.assign({
...
Game.prototype.placementListener = function(e) {
	...
	if (self.placingOnGrid) {
		...
		if (successful) {
			var replayProps = window.getSessionReplayProps ? window.getSessionReplayProps() : {};
			analytics.track('Ship Placed', Object.assign({
				Ship: Game.placeShipType,
				Success: true,
				X: x,
				Y: y,
...
Game.prototype.toggleRotation = function(e) {
	...
	var replayProps = window.getSessionReplayProps ? window.getSessionReplayProps() : {};
	analytics.track('Ship Rotated', Object.assign({
		Ship: Game.placeShipType
	}, replayProps));
...
Game.prototype.startGame = function(e) {
	...
	var replayProps = window.getSessionReplayProps ? window.getSessionReplayProps() : {};
	analytics.track('Game Started', replayProps);
};
